...
void print_control_mode(void)
{
	switch (control_mode){
		...
		case LOITER:
			...
			break;
		case KIRBY_SPECIAL:                              //FOR THE NEW MODE ADDED
			Serial.println("##8| KIRBY_SPECIAL");
			break;
	}
	print_new_wp_info();
}
...
{
	Serial.print("QQQ");     // SWITCHED FROM +++ TO QQQ SO THAT THE XBEE DOESNT GET REPROGRAMMED!
	Serial.print("ASP:");
	...
	Serial.print(pitch_sensor/100,DEC);
        Serial.print (",sR:");
	Serial.print(servo_roll,DEC);     //ADDED FOR GROUND STATION 
        Serial.print (",sP:");
	Serial.print(servo_pitch,DEC);    //ADDED FOR GROUND STATION 
        Serial.print (",sT:");
	Serial.print(servo_throttle,DEC); //ADDED FOR GROUND STATION 
        Serial.print (",STT:");
